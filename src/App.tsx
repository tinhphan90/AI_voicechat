...
export default function App() {
    ...
    ws.onmessage = (event) => {
      try {
        ...
        if (msg.type === "status") {
          ...
          console.log("[App] Car Tool Call received:", msg.name, msg.args);
          ...
          if (toolName === "navigate_to") {
            ...
            actionLabel = `🚗 Đang mở bản đồ chỉ đường đến: ${dest}`;
            window.open(`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(dest)}`, "_blank");
          } else if (toolName === "open_youtube") {
            const query = args.query || "";
            actionLabel = `▶️ Đang mở YouTube tìm kiếm: ${query}`;
            window.open(`https://www.youtube.com/results?search_query=${encodeURIComponent(query)}`, "_blank");
          } else if (toolName === "play_music") {
            const song = args.song_name || "";
            actionLabel = `🎵 Đang phát nhạc: ${song}`;
            window.open(`https://zingmp3.vn/tim-kiem/tat-ca?q=${encodeURIComponent(song)}`, "_blank");
          }
  ...
  useEffect(() => {
    ...
    const handleOnline = () => {
      console.log("[App] Network is back ONLINE. Reconnecting WebSocket immediately...");
      if (reconnectTimeoutRef.current) clearTimeout(reconnectTimeoutRef.current);
      if (wsRef.current) {
        try {
          wsRef.current.close();
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col font-sans selection:bg-indigo-500 selection:text-white">
          ...
          <div className="min-w-0">
            <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
              ...
              <button
                ...
                title="Xem thông tin mốc neo Phiên bản V1.2"
              >
      ...
      <main className="flex-1 max-w-7xl w-full mx-auto p-2.5 sm:p-4 md:p-6 flex flex-col gap-3 sm:gap-5">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-5 flex flex-col gap-5">
            {/* Visualizer Orb with Center Huge Mic Button */}
            <VoiceVisualizer
            ...
            {/* Quick Car & Assistant Action Shortcuts (One-tap for car/mobile users) */}
            <div className="p-3 bg-slate-900/90 rounded-2xl border border-slate-800 backdrop-blur-md">
              <span className="text-[11px] font-bold text-slate-400 uppercase tracking-wider block mb-2 px-1">
                🚗 Lệnh nhanh Ô tô & Di động:
              </span>
              <div className="flex flex-wrap gap-2">
                {[
                  ...
                  <button
                    ...
                    className="px-3 py-1.5 rounded-full bg-slate-800/90 hover:bg-indigo-950 hover:border-indigo-500/80 border border-slate-700/80 text-xs text-slate-200 font-medium transition-all shadow-sm active:scale-95"
                  >
                    {promptText}
                  </button>
                ))}
            ...
            <div className="p-3.5 bg-slate-900/80 rounded-2xl border border-slate-800/80 backdrop-blur-md flex items-center justify-around gap-3">
              {/* Mic Toggle Button */}
              <button
                onClick={toggleMicrophone}
                disabled={connectionState !== "connected"}
                className={`flex-1 flex items-center justify-center gap-2.5 py-4 px-4 rounded-xl font-bold text-xs sm:text-sm transition-all shadow-lg ${
                  isMicActive
                    ? "bg-rose-600 hover:bg-rose-500 text-white shadow-rose-600/30 animate-pulse border border-rose-400"
      ...
      {/* Version V1.2 Release Baseline & Anchor Point Modal */}
      {isVersionModalOpen && (
          ...
          <div className="relative w-full max-w-xl bg-slate-900 rounded-2xl border border-slate-800 shadow-2xl overflow-hidden flex flex-col">
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  <div className="flex items-center gap-2">
                    <h3 className="text-lg font-bold text-white">Phiên Bản V1.2 - Car Assistant & Function Calling</h3>
                    <span className="px-2 py-0.5 rounded-full text-[10px] font-bold bg-emerald-500/20 text-emerald-300 border border-emerald-500/40">
                      Anchor Point V1.2
                    </span>
                  </div>
                  <p className="text-xs text-slate-400">Trợ lý ô tô thông minh Kiki Auto - Dẫn đường, mở YouTube, phát nhạc tự động</p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 space-y-4 max-h-[70vh] overflow-y-auto text-xs sm:text-sm text-slate-300">
              <div className="p-3.5 rounded-xl bg-emerald-950/40 border border-emerald-800/40 text-emerald-200 text-xs flex items-start gap-2.5">
                ...
                <div>
                  <span className="font-bold">Điểm neo hệ thống V1.2 đã khởi tạo thành công!</span> Tích hợp Gemini Live Function Calling kết hợp Android Intents điều khiển ứng dụng ô tô native tức thì.
                </div>
              </div>

              <div className="space-y-3">
                <h4 className="font-semibold text-slate-200 text-xs uppercase tracking-wider text-indigo-400">
                  🚗 Nâng cấp nổi bật Phiên Bản V1.2 (Car Assistant)
                </h4>
                <ul className="space-y-2 text-xs">
                  <li className="flex items-start gap-2 p-2.5 rounded-xl bg-slate-950/60 border border-slate-800">
                    <Sparkles className="w-4 h-4 text-emerald-400 flex-shrink-0 mt-0.5" />
                    <div>
                      <span className="font-bold text-slate-200">Dẫn đường thông minh (navigate_to):</span> Tự động kích hoạt Google Maps / VietMap chỉ đường theo khẩu lệnh địa điểm.
                    </div>
                  </li>
                  <li className="flex items-start gap-2 p-2.5 rounded-xl bg-slate-950/60 border border-slate-800">
                    <Activity className="w-4 h-4 text-indigo-400 flex-shrink-0 mt-0.5" />
                    <div>
                      <span className="font-bold text-slate-200">Mở YouTube & Video (open_youtube):</span> Tìm kiếm và phát clip/nhạc trên YouTube theo từ khóa tài xế yêu cầu.
                    </div>
                  </li>
                  <li className="flex items-start gap-2 p-2.5 rounded-xl bg-slate-950/60 border border-slate-800">
                    <UploadCloud className="w-4 h-4 text-cyan-400 flex-shrink-0 mt-0.5" />
                    <div>
                      <span className="font-bold text-slate-200">Phát nhạc Android (play_music):</span> Điều khiển trình phát nhạc Zing MP3 / Spotify / MediaStore rảnh tay khi lái xe.
                    </div>
                  </li>
                </ul>
              </div>
