...
export class AudioRecorder {
  ...
  public async start(): Promise<void> {
    ...
    try {
      // 1. Request microphone stream with echo cancellation and noise suppression (with iOS / retry fallback)
      ...
      try {
        ...
        console.warn("[AudioRecorder] Detailed audio constraints failed, retrying with basic { audio: true }:", firstErr);
        try {
          // Wait 250ms in case previous audio stream is being released
          await new Promise((r) => setTimeout(r, 250));
          stream = await navigator.mediaDevices.getUserMedia({ audio: true });
        } catch (secondErr: any) {
          if (secondErr.name === "NotReadableError" || secondErr.name === "TrackStartError") {
            console.warn("[AudioRecorder] Mic busy, waiting 500ms for hardware release retry...");
      ...
      if (err.name === "NotAllowedError" || err.name === "PermissionDeniedError") {
        throw new Error("PERMISSION_DENIED: Bạn đã từ chối quyền Micro. Vui lòng cho phép quyền trong cài đặt.");
      } else if (err.name === "NotFoundError" || err.name === "DevicesNotFoundError") {
        throw new Error("NO_MIC_FOUND: Không tìm thấy thiết bị Microphone trên máy.");
      } else if (err.name === "NotReadableError" || err.name === "TrackStartError") {
        throw new Error("MIC_IN_USE: Microphone đang bị chiếm dụng bởi ứng dụng khác (Zalo, Messenger, Meet, Zoom, TikTok, Siri hoặc tab trình duyệt khác). Vui lòng đóng ứng dụng đó và thử lại.");
